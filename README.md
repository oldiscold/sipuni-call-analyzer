...
3. Укажите адрес интеграции (HTTP API):
   - **URL**: `https://your-domain.railway.app/webhook`
   - **Метод**: Сипуни шлёт **GET** с параметрами в query-строке (см. HTTP API).
     Приложение также принимает POST (form-urlencoded) как fallback.
   - Отдельно выбирать «только завершение звонка» не нужно — Сипуни присылает
     все события (event=1 начало, 2 завершение, 3 ответ, 4 промежуточное).
     Приложение само обрабатывает только отвеченные звонки с записью.
   - **Запись разговоров** должна быть включена, иначе не придёт `call_record_link`.
4. Сохраните и скопируйте API-ключ для переменной `SIPUNI_API_KEY`

> ⚠️ Важно: приложение **всегда** должно отвечать ровно `{"success": true}`.
> Если Сипуни получит иной ответ (404, 307-редирект, 500), она **приостановит**
> отправку вебхуков. Чтобы возобновить — удалите и заново добавьте URL в панели.
